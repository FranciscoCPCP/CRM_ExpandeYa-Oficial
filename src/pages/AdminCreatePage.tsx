...
const AdminCreatePage: React.FC<Props> = ({ admin, onAdminCreated, onCancel }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!admin) {
        const userRes = await fetch('http://192.168.10.19:8000/api/register', {
          method: 'POST',
          headers: {
      ...
      const res = await fetch(admin ? `http://192.168.10.19:8002/api/administrador/${admin.id}` : 'http://192.168.10.19:8002/api/administrador', {
